...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const savedLastReview = localStorage.getItem('lastReviewDate');
    const savedCardsReviewed = localStorage.getItem('cardsReviewed');

    if (savedXp) setXp(parseInt(savedXp));
    if (savedStreak) setStreak(parseInt(savedStreak));
    if (savedLastReview) setLastReviewDate(savedLastReview);
    if (savedCardsReviewed) setCardsReviewed(parseInt(savedCardsReviewed));
  }, []);

  // Update streak based on daily reviews
  useEffect(() => {
    const today = new Date().toDateString();
    
    if (lastReviewDate !== today) {
      // If last review was not today, check if it was yesterday
      ...
      if (lastReviewDate === yesterday.toDateString()) {
        ...
        // If last review was not yesterday, reset streak
        setStreak(1);
      }
      
      setLastReviewDate(today);
    }
  }, [cardsReviewed, lastReviewDate]);

  // Save progress to localStorage whenever it changes
  useEffect(() => {
    localStorage.setItem('flashcardXp', xp.toString());
    localStorage.setItem('flashcardStreak', streak.toString());
    localStorage.setItem('lastReviewDate', lastReviewDate || '');
    localStorage.setItem('cardsReviewed', cardsReviewed.toString());
  }, [xp, streak, lastReviewDate, cardsReviewed]);
  ...
  const handleNextCard = () => {
    // First reset the current card
    setShouldResetCard(true);
    
    // Wait for the reset animation to complete before changing the card
    setTimeout(() => {
